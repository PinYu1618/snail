...
pub fn suspend_current_hart(hsm: &Hsm) {
    ...
    state_lock
        .entry(hart_id)
        .insert(AtomicU8::new(HartState::Suspended as u8));
    drop(state_lock);
    ...
    state_lock
        .entry(hart_id)
        .insert(AtomicU8::new(HartState::ResumePending as u8));
    drop(state_lock);
    // resume
    if !prev_msoft {
...
impl Hsm {
    ...
    pub(crate) fn record_current_stop_finished(&self) {
        ...
        self.state
            .lock()
            .entry(hart_id)
            .insert(AtomicU8::new(HartState::Stopped as u8));
    }
    ...
    pub(crate) fn record_current_start_finished(&self) {
        ...
        self.state
            .lock()
            .entry(hart_id)
            .insert(AtomicU8::new(HartState::Started as u8));
    }
}
...
impl rustsbi::Hsm for Hsm {
    ...
    fn hart_start(&self, hart_id: usize, start_addr: usize, opaque: usize) -> SbiRet {
        ...
        let current_state = state_lock
            .entry(hart_id)
            .or_insert(AtomicU8::new(HartState::Stopped as u8))
            .compare_exchange(
                HartState::Stopped as u8,
                HartState::StartPending as u8,
                Ordering::AcqRel,
                Ordering::Acquire,
            );
        ...
        if current_state == Err(HartState::Started as u8) {
        ...
        if current_state != Ok(HartState::Stopped as u8) {
            return SbiRet::invalid_param();
        }
    ...
    fn hart_stop(&self, hart_id: usize) -> SbiRet {
        ...
        let current_state = state_lock
            ...
            .or_insert(AtomicU8::new(HartState::Stopped as u8))
            .compare_exchange(
                HartState::Started as u8,
                HartState::StopPending as u8,
                Ordering::AcqRel,
                Ordering::Acquire,
            );
        ...
        match suspend_type {
            ...
            SUSPEND_RETENTIVE => {
                ...
                let current_state = state_lock
                    ...
                    .compare_exchange(
                        HartState::Started as u8,
                        HartState::SuspendPending as u8,
                        Ordering::AcqRel,
                        Ordering::Acquire,
                    );
                // check current hart state
